// Hierarchical Ricker model + AR1 error term + covariate
//
// Parameters:
...
transformed parameters {
    ...
    for(i in 1:n_series) {
        ...
        yhat[y_start[i]] = alpha[i] + beta[i] * x1[y_start[i]] + gamma[i] * x2[y_start[i]];
        epsilon[y_start[i]] = y[y_start[i]] - yhat[y_start[i]];

        for(t in (y_start[i]+1):y_end[i]) {
            tmp_epsilon = epsilon[t-1];
            yhat[t] = alpha[i] + beta[i] * x1[t] + gamma[i] * x2[t] + phi * tmp_epsilon;
