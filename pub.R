...
gamma <- summary(hb07r2, pars = "mu_gamma")$summary
kappa <- summary(hb07r2, pars = "mu_kappa")$summary
chi   <- summary(hb07r2, pars = "mu_chi")$summary
...
write.csv(tab.coef, file = "./pub/model_coefficients.csv")

## Tables for SI -------------------------------------------

## model fits
fits <- list(main = hb07r2,
             total_abundance = hb07tn,
             total_biomass = hb07tb,
             north_america_total_abundance = hb07tna,
             north_america_pink_abundance = hb07na,
             geographic_weights = hb07g,
             detrended_pinks = hb07e,
             post89 = hb07r,
             full = hb07a,
             ocean_region = hb07oc)

## model R^2
r2 <- list(main = r2.hb07r2,
           total_abundance = r2.hb07tn,
           total_biomass = r2.hb07tb,
           north_am_total_abundance = r2.hb07tna,
           north_america_pink_abundance = r2.hb07na,
           geograpic_weights = r2.hb07g,
           detrended_pinks = r2.hb07e,
           post89 = r2.hb07r,
           full = r2.hb07a,
           ocean_region = r2.hb07oc)


## 1. RMSE and R^2 for each model
rmse.lst <- lapply(seq_along(fits), function(i) {
    nam <- names(fits)[i]
    y <- sock$lnRS[sock$BY >= 1975]
    if(nam == "post89") y <- sock$lnRS[sock$BY >= 1987]
    if(nam == "full") y <- sock$lnRS
    mat   <- as.matrix(fits[[i]], pars = "yhat")
    resid <- sweep(mat, 2, y, FUN = "-") * -1  ## residuals
    rmse  <- sqrt(mean(resid*resid))
    data.frame(model = nam, rmse = rmse, r2 = mean(r2[[i]]))
})
rmse <- plyr::rbind.fill(rmse.lst)
write.csv(rmse, file = "./pub/si_rmse_models.csv")


## 2. Regional level params and CIs for each model
region.tbl <- lapply(seq_along(fits), function(i) {
    probs <- c(0.025, 0.975)
    sst      <- as.data.frame(summary(fits[[i]], pars = "mu_gamma", probs = probs)[[1]])
    comp     <- as.data.frame(summary(fits[[i]], pars = "mu_kappa", probs = probs)[[1]])
    interact <- as.data.frame(summary(fits[[i]], pars = "mu_chi", probs = probs)[[1]])
    data.frame(model = names(fits)[i],
               region = unique(sock.info$Ocean.Region),
               sst_mean = sst[["mean"]],
               sst_lowerCI = sst[["2.5%"]],
...
region.tbl <- plyr::rbind.fill(region.tbl)
write.csv(region.tbl, file = "./pub/si_regional_params.csv")


## 3. Stock-specific params and CIs of main model
probs <- c(0.025, 0.975)
alpha    <- as.data.frame(summary(hb07r2, pars = "alpha", probs = probs)[[1]])
beta     <- as.data.frame(summary(hb07r2, pars = "beta", probs = probs)[[1]])
sigma    <- as.data.frame(summary(hb07r2, pars = "sigma", probs = probs)[[1]])
sst      <- as.data.frame(summary(hb07r2, pars = "gamma", probs = probs)[[1]])
comp     <- as.data.frame(summary(hb07r2, pars = "kappa", probs = probs)[[1]])
interact <- as.data.frame(summary(hb07r2, pars = "chi", probs = probs)[[1]])
...
write.csv(stock.tbl, file = "./pub/si_main_model_stock_params.csv")



## Fig: Map + covars ---------------------------------------

pdf("./pub/fig_map_covars.pdf",
     width = 5, height = 6.0)
...
for(k in 1:2 ) {  ## loop over covars
    ...
    for(i in rev(seq_along(oc.reg))) {  ## loop over regions
        ...
        if(k == 1) {
            x <- lapply(split(dat.i, dat.i$Stock), function(x) {
                        lines(x$BY, x[ , "np_pinks_hatch_sec"] / scale[k], col = col.stock[i], lty = 2)
            })
            ph <- plyr::ddply(dat.i, .(BY), function(x) {
                min  <- min(x[ , "np_pinks_hatch_sec"] / scale[k], na.rm = TRUE)
                max  <- max(x[ , "np_pinks_hatch_sec"] / scale[k], na.rm = TRUE)
                mean <- mean(x[ , "np_pinks_hatch_sec"] / scale[k], na.rm = TRUE)
                data.frame(min = min, max = max, mean = mean)
            })
...
gamma.stock <- hb_param_df(hb07r2, "gamma", "Ocean.Region", "SST")
kappa.stock <- hb_param_df(hb07r2, "kappa", "Ocean.Region", "Comp")
chi.stock   <- hb_param_df(hb07r2, "chi", "Ocean.Region", "SST x Comp")
...
pdf("./pub/Sfig_coef_dot_main.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()



## Fig: dot + density geographic ---------------------------
gamma.stock <- hb_param_df(hb07g, "gamma", "Ocean.Region", "SST")
kappa.stock <- hb_param_df(hb07g, "kappa", "Ocean.Region", "Comp")
chi.stock   <- hb_param_df(hb07g, "chi", "Ocean.Region", "SST x Comp")
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
df.mu <- plyr::ddply(df.dot, .(region, var), summarize,
...
pdf("./pub/Sfig_coef_dot_geo.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()



## Fig: dot + density detrend ------------------------------
gamma.stock <- hb_param_df(hb07e, "gamma", "Ocean.Region", "SST")
kappa.stock <- hb_param_df(hb07e, "kappa", "Ocean.Region", "Comp")
chi.stock   <- hb_param_df(hb07e, "chi", "Ocean.Region", "SST x Comp")
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
df.mu <- plyr::ddply(df.dot, .(region, var), summarize,
...
pdf("./pub/Sfig_coef_dot_detrend.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()



## Fig: dot + density 88/89 --------------------------------
gamma.stock <- hb_param_df(hb07r, "gamma", "Ocean.Region", "SST",
                           sock.info[sock.info$yr_end >= 1987, ])
kappa.stock <- hb_param_df(hb07r, "kappa", "Ocean.Region", "Comp",
                           sock.info[sock.info$yr_end >= 1987, ])
chi.stock   <- hb_param_df(hb07r, "chi", "Ocean.Region", "SST x Comp",
                           sock.info[sock.info$yr_end >= 1987, ])
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
...
gamma.stock <- hb_param_df(hb07a, "gamma", "Ocean.Region", "SST",
                           sock.info[sock.info$yr_end >= 1975, ])
kappa.stock <- hb_param_df(hb07a, "kappa", "Ocean.Region", "Comp",
                           sock.info[sock.info$yr_end >= 1975, ])
chi.stock   <- hb_param_df(hb07a, "chi", "Ocean.Region", "SST x Comp",
                           sock.info[sock.info$yr_end >= 1975, ])
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
...
pdf("./pub/Sfig_coef_dot_full.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()



## Fig: dot + density NA pinks -----------------------------
gamma.stock <- hb_param_df(hb07na, "gamma", "Ocean.Region", "SST")
kappa.stock <- hb_param_df(hb07na, "kappa", "Ocean.Region", "Comp")
chi.stock   <- hb_param_df(hb07na, "chi", "Ocean.Region", "SST x Comp")
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
df.mu <- plyr::ddply(df.dot, .(region, var), summarize,
...
pdf("./pub/Sfig_coef_dot_na_pinks.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()
...
## Fig: dot + density total comp np numbers ----------------
gamma.stock <- hb_param_df(hb07tn, "gamma", "Ocean.Region", "SST")
kappa.stock <- hb_param_df(hb07tn, "kappa", "Ocean.Region", "Comp")
chi.stock   <- hb_param_df(hb07tn, "chi", "Ocean.Region", "SST x Comp")
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
df.mu <- plyr::ddply(df.dot, .(region, var), summarize,
...
pdf("./pub/Sfig_coef_dot_total_comp_np_numbers.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()



## Fig: dot + density total comp np biomass ----------------
gamma.stock <- hb_param_df(hb07tb, "gamma", "Ocean.Region", "SST")
kappa.stock <- hb_param_df(hb07tb, "kappa", "Ocean.Region", "Comp")
chi.stock   <- hb_param_df(hb07tb, "chi", "Ocean.Region", "SST x Comp")
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
df.mu <- plyr::ddply(df.dot, .(region, var), summarize,
...
pdf("./pub/Sfig_coef_dot_total_comp_np_biomass.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()



## Fig: dot + density total comp na numbers ----------------
gamma.stock <- hb_param_df(hb07tna, "gamma", "Ocean.Region", "SST")
kappa.stock <- hb_param_df(hb07tna, "kappa", "Ocean.Region", "Comp")
chi.stock   <- hb_param_df(hb07tna, "chi", "Ocean.Region", "SST x Comp")
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
df.mu <- plyr::ddply(df.dot, .(region, var), summarize,
...
pdf("./pub/Sfig_coef_dot_total_comp_na_numbers.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()
...
kappa.stock <- hb_param_df(hb07oc, "kappa", "Ocean.Region2", "Comp")
chi.stock   <- hb_param_df(hb07oc, "chi", "Ocean.Region2", "SST x Comp")
df.dot <- rbind(gamma.stock, kappa.stock, chi.stock)
df.dot <- ocean_region_lab(df.dot, "region", FALSE)
df.dot$var <- factor(df.dot$var, levels = c("SST", "Comp", "SST x Comp"))
df.mu <- plyr::ddply(df.dot, .(region, var), summarize,
...
pdf("./pub/Sfig_coef_dot_seak_goa.pdf", width = 6.5, height = 6.0)
    print(g)
dev.off()
