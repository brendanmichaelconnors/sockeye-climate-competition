## Clean-up the downloaded sockeye data
##
## This script cleans/processes the raw downloaded sockeye data. The output of
## this script is a master brood table for the anlysis and a summary info table.



## Read in downloaded data
s.brood <- read.table("./data-downloaded/raw_brood_table_2019_07_16.csv",
                      sep = ",",skip = 0,
                      na.string = c("NA", "ND", "Canadians:  Please check the G-R ages.  "),
                      stringsAsFactors = FALSE, header = TRUE)

s.info <- read.table("./data-downloaded/raw_stock_info_2019_03_31.csv", sep = ",",
...
bt <- ddply(s.brood.use, c("Stock.ID", "BY"),function(x) {
	R <- sum(x[ , grep("^R[[:digit:]]\\.[[:digit:]]", names(x))], na.rm = TRUE) # total recruits
	S <- x$TotalEscapement # spawners
	ocean_0 <- sum(x[,grep("^R0\\.", names(x))],na.rm=T)/R
	ocean_1 <- sum(x[,grep("^R1\\.", names(x))],na.rm=T)/R
	ocean_2 <- sum(x[,grep("^R2\\.", names(x))],na.rm=T)/R
	ocean_3 <- sum(x[,grep("^R3\\.", names(x))],na.rm=T)/R
	ocean_4 <- sum(x[,grep("^R4\\.", names(x))],na.rm=T)/R
	R0.1 <- x[,"R0.1"]/R
	R0.2 <- x[,"R0.2"]/R
	R0.3 <- x[,"R0.3"]/R
	R0.4 <- x[,"R0.4"]/R
    ...
    data.frame(Stock = x$Stock,
               ...
               Lat = x$Lat,
               Lon = x$Lon,
               R, S, ocean_0, ocean_1, ocean_2, ocean_3, ocean_4, R0.1, R0.2, R0.3, R0.4,
...
bt.out.3 <- subset(bt.out.2,BY < 2010) # do this becasue 2009 is the last brood year we have complete competitor index for
